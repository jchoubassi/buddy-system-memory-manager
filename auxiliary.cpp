...
//this function generates a random number based on the seed value
int myrand() {
    seed = (seed * 2416 + 374441) % 1095976;
    return seed;
}

//this function generates a random size based on the random number generated by myrand()
int randomsize() {
    int k = myrand();
    int j = (k & 3) + ((k >> 2) & 3) + ((k >> 4) & 3) + ((k >> 6) & 3) + ((k >> 4) & 3) + ((k >> 4) & 3);
    j = 1 << j;
    return 500 + (myrand() % (j << 5));
}
...
//this function generates a random size based on the random number generated by myrand()
int randomsize() {
	int k = myrand(); // Generate a random number
	int j = (k & 3) + ((k >> 2) & 3) + ((k >> 4) & 3) + ((k >> 6) & 3) + ((k >> 8) & 3) + ((k >> 10) & 3); // Adjusted to use more bits for larger sizes
	j = 1 << j; //j is now a power of 2 based on the bits extracted from k
	return (myrand() % j) + 1; //return a random size between 1 and j
}
